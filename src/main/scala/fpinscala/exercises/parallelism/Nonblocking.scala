...
object Nonblocking:
  ...
  object Par:
    ...
    def choiceN[A](p: Par[Int])(ps: List[Par[A]]): Par[A] =
      es => cb => p(es)(idx => eval(es)(ps(idx % ps.length)(es)(cb)))

    def choiceViaChoiceN[A](p: Par[Boolean])(ifTrue: Par[A], ifFalse: Par[A]): Par[A] =
      choiceN(map(p)(b => if (b) 0 else 1))(List(ifTrue, ifFalse))

    def choiceMap[K, V](p: Par[K])(ps: Map[K, Par[V]]): Par[V] =
      es => cb => p(es)(k => ps(k)(es)(cb))
    
    def choiceMapViaFlatMap[K, V](p: Par[K])(ps: Map[K, Par[V]]): Par[V] = 
      flatMap(p)(k => ps(k))
    ...
    def choiceViaChooser[A](p: Par[Boolean])(ifTrue: Par[A], ifFalse: Par[A]): Par[A] = 
      chooser(p)(b => if b then ifTrue else ifFalse)

    def choiceNviaChooser[A](n: Par[Int])(ps: List[Par[A]]): Par[A] = 
      chooser(n)(idx => ps(idx % ps.length))
    ...
    def join[A](pp: Par[Par[A]]): Par[A] =
      es => cb => pp(es)(pa => eval(es)(pa(es)(cb)))
