...
class StateSuite extends PropSuite:
  ...
  test("State.traverse")(genStringList): list =>
    val half = list.length / 2
    // return State as getter head from list' State
    val f: String => State[List[String], Option[String]] = _ => stateA
    val (result, rest) = traverse(list.take(half))(f).run(list)
    val (first, restList) = list.splitAt(half)
    assertEquals(result, first.map(Some(_)))
    assertEquals(rest, restList)
